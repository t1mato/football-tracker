"""Connector-agnostic query layer: a read-only DuckDB/BigQuery connection
and one function per page's data need, with zero framework dependency.

Read-only, always -- this project's DuckDB allows one writer or many
readers, never both (see CLAUDE.md). Nothing that imports this module may
hold a write lock.

Callers own caching and connection lifecycle: app/queries.py wraps every
function here in @st.cache_data for the Streamlit app; backend/main.py
creates one connection at FastAPI startup and reuses it for the process
lifetime. Neither concern belongs in this file.
"""
